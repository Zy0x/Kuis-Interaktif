...
export const QuizResult: React.FC<QuizResultProps> = ({
  ...
  // Stars calculation: 100 = 3, >= 70 = 2, else 1
  let stars = 1;
  ...
  // Trigger celebration & record attempt
  useEffect(() => {
    playCelebration();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-sky-50/40 to-indigo-50/40 p-4 sm:p-6 pb-28">
      <div className="max-w-2xl mx-auto space-y-6">
        
        {/* Celebration Banner Card */}
        <div className="bg-white rounded-3xl p-6 sm:p-8 border-4 border-amber-300 shadow-2xl text-center relative overflow-hidden animate-pop-in">
          ...
          <div className="grid grid-cols-3 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
            <div className="text-center">
              ...
              <p className="text-2xl sm:text-3xl font-black text-blue-600 mt-0.5">{score}</p>
            </div>
            <div className="text-center border-x border-slate-200">
              <span className="text-xs font-bold text-slate-500">Jawaban Benar</span>
              <p className="text-2xl sm:text-3xl font-black text-emerald-600 mt-0.5">
                {correctCount} / {totalCount}
              </p>
            </div>
            <div className="text-center">
              <span className="text-xs font-bold text-slate-500">Waktu Main</span>
              <p className="text-2xl sm:text-3xl font-black text-amber-600 mt-0.5">{totalTimeSpent}s</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex rounded-2xl bg-white p-1.5 border border-slate-200 shadow-sm">
          <button
            ...
            className={`flex-1 py-3 rounded-xl font-black text-xs sm:text-sm flex items-center justify-center gap-1.5 transition-all min-h-[48px] ${
              activeTab === 'review'
          ...
          <div className="space-y-4 animate-pop-in">
            ...
            {quiz.questions.map((q, idx) => {
              ...
              return (
                <div
                  key={q.id}
                  className={`bg-white rounded-2xl p-5 border-2 shadow-sm space-y-3 ${
                ...
                >
                  ...
                  <div className="text-xs text-blue-900 bg-blue-50/70 p-3 rounded-xl border border-blue-100">
        ...
        {/* Tab 2: Leaderboard */}
        {activeTab === 'leaderboard' && (
          <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm space-y-4 animate-pop-in">
                ...
                return (
                  <div key={entry.id} className="py-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="w-7 flex justify-center">{medalBadge}</div>
                      <div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 p-3.5 z-30 shadow-2xl">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <button
            ...
            className="flex-1 py-3 px-4 rounded-2xl font-black text-xs sm:text-sm text-slate-700 bg-slate-100 hover:bg-slate-200 flex items-center justify-center gap-1.5 min-h-[48px] btn-playful"
          >
          ...
          <button
            onClick={handleShare}
            className="p-3 rounded-2xl bg-amber-100 text-amber-900 hover:bg-amber-200 min-h-[48px] min-w-[48px] flex items-center justify-center btn-playful"
            title="Bagikan Skor"
