...
export const QuizArena: React.FC<QuizArenaProps> = ({
    ...
    timerRef.current = window.setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          handleAnswerSelect(-1); // -1 = timeout
          return 0;
        }
        return prev - 1;
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between select-none">
      ...
      {/* Top Header */}
      <header className="bg-white border-b border-slate-200 px-4 py-2.5 shadow-sm sticky top-0 z-20">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="p-2 text-slate-500 hover:text-slate-900 rounded-xl min-h-[44px] min-w-[44px] flex items-center justify-center"
          ...
          <div className="text-center flex-1">
            ...
            <h2 className="text-xs sm:text-sm font-extrabold text-slate-700 truncate mt-0.5">
              {quiz.title}
            </h2>
          </div>
      ...
      {/* Main Arena Content */}
      <main className="flex-1 max-w-2xl w-full mx-auto p-4 sm:p-6 flex flex-col justify-center space-y-4">
        ...
        {/* Question Card */}
        <div className="bg-white rounded-3xl p-5 sm:p-7 border-2 border-slate-200/80 shadow-playful space-y-4 animate-pop-in">
          ...
          {question.imageCaption && (
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200/80 rounded-2xl p-4 sm:p-6 text-center shadow-inner">
              <div className="text-4xl sm:text-5xl select-none tracking-widest mb-2 animate-bounce-soft">
                {question.imageCaption}
              </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs sm:text-sm ${
                    ...
                    <span className="text-sm sm:text-base font-bold">{optText}</span>
                  </div>
          ...
          {/* Explanation Box (Visible after answer confirmed) */}
          {isAnswerConfirmed && (
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-2xl space-y-1 animate-pop-in">
      ...
      <footer className="bg-white border-t border-slate-200 p-4 sticky bottom-0 z-20">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          ...
          <button
            disabled={!isAnswerConfirmed}
            onClick={handleNext}
            className={`w-full xs:w-auto px-6 py-3 rounded-2xl font-black text-sm flex items-center justify-center gap-2 transition-all min-h-[48px] btn-playful ${
              isAnswerConfirmed
