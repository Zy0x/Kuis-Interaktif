...
export const QuizCreator: React.FC<QuizCreatorProps> = ({
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-b from-amber-50/70 via-sky-50/40 to-indigo-50/40 pb-20 select-none flex flex-col">
      
      {/* Top Header Navigation - Fluid */}
      <header className="w-full sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-amber-100 px-3 sm:px-6 lg:px-12 py-3 shadow-sm">
        <div className="w-full max-w-[2200px] mx-auto flex items-center justify-between gap-3">
          <button
            onClick={() => {
              playClick();
              onBack();
            }}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs sm:text-sm min-h-[44px] sm:min-h-[48px] btn-playful"
          >
          ...
          <div className="w-24 flex justify-end">
            <span className="text-xs font-black bg-amber-100 text-amber-800 px-3 py-1.5 rounded-full">
        ...
        {/* Step Indicator Tabs */}
        <div className="w-full max-w-[2200px] mx-auto mt-2.5 grid grid-cols-3 gap-2 sm:gap-3">
          <button
            onClick={() => {
              playClick();
              setCurrentStep(1);
            }}
            className={`py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all min-h-[44px] ${
              currentStep === 1
      ...
      {/* Main Container - Fluid width up to 2200px */}
      <main className="w-full max-w-[2200px] mx-auto px-3 sm:px-6 lg:px-12 pt-4 sm:pt-6 flex-1">

        {/* ================= STEP 1: GENERAL INFO ================= */}
        {currentStep === 1 && (
          <div className="w-full max-w-4xl mx-auto bg-white rounded-3xl p-5 sm:p-8 border-2 border-slate-200 shadow-playful space-y-6 animate-pop-in">
            <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100">
              <BookOpen className="w-5 h-5 text-blue-600" />
              <h2 className="text-lg sm:text-xl font-black text-slate-900">Informasi Kuis Baru</h2>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-4 py-3 rounded-2xl border-2 border-slate-300 focus:border-blue-500 focus:outline-none font-bold text-sm"
                />
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {EMOJI_OPTIONS.map((em) => (
                    <button
                      ...
                      className={`w-11 h-11 sm:w-12 sm:h-12 rounded-2xl text-2xl flex items-center justify-center border-2 transition-transform ${
                        coverEmoji === em
        ...
        {/* ================= STEP 2: BANK SOAL & GAMBAR (Dual Column on Desktop/Ultrawide) ================= */}
        {currentStep === 2 && (
          <div className="w-full space-y-6 animate-pop-in">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              
              {/* Left Column (Lg: 5 cols): Question Bank List */}
              <div className="lg:col-span-5 bg-white rounded-3xl p-5 border-2 border-slate-200 shadow-sm space-y-3">
                ...
                {questions.length === 0 ? (
                  <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-300 p-4">
                    <p className="text-xs text-slate-500 font-bold">Belum ada soal. Tulis soal pertamamu pada formulir di sebelah kanan!</p>
                  </div>
                ) : (
                  <div className="space-y-2.5 max-h-[65vh] overflow-y-auto pr-1">
                    {questions.map((q, idx) => (
                      <div
                        key={q.id}
                        className="p-3.5 rounded-2xl border border-slate-200 bg-slate-50 flex items-start justify-between gap-3"
                      >
                        <div className="space-y-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {q.imageCaption && (
                              <span className="text-[11px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full font-bold truncate max-w-[120px]">
                                {q.imageCaption}
                              </span>
                            )}
                          </div>
                          <p className="font-bold text-slate-800 text-xs sm:text-sm line-clamp-2 break-words">{q.text}</p>
                          <p className="text-xs text-emerald-700 font-extrabold truncate">
                        ...
                        <button
                          ...
                          className="p-2 text-rose-500 hover:bg-rose-50 rounded-xl min-h-[40px] min-w-[40px] flex items-center justify-center flex-shrink-0"
                          title="Hapus Soal"
                        >
              ...
              {/* Right Column (Lg: 7 cols): Form to Add Question */}
              <div className="lg:col-span-7">
                    ...
                    <div className="space-y-2.5 pt-1">
                      ...
                      {qOptions.map((opt, idx) => {
                        ...
                        return (
                          ...
                          {isCorrect && (
                            <span className="text-xs font-black text-emerald-700 px-2 py-1 bg-emerald-100 rounded-lg whitespace-nowrap">
                              Kunci Benar
                            </span>
                          )}
                        </div>
                ...
                <div className="text-center py-12 bg-white rounded-3xl border-2 border-slate-200 p-6 space-y-3">
                  <p className="font-bold text-slate-700 text-sm">Formulir soal sedang ditutup.</p>
                  <button
                    onClick={() => {
                      playClick();
                      setIsAddingQuestion(true);
                    }}
                    className="px-5 py-3 rounded-2xl bg-blue-600 text-white font-black text-xs sm:text-sm inline-flex items-center gap-2 min-h-[48px]"
                  >
                    <Plus className="w-4 h-4" /> Buka Formulir Tambah Soal
                  </button>
                </div>
              )}
            </div>
        ...
        <div className="w-full max-w-4xl mx-auto bg-white rounded-3xl p-5 sm:p-8 border-2 border-slate-200 shadow-playful space-y-6 animate-pop-in">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg sm:text-xl font-black text-slate-900">Pratinjau Kuis Sebelum Diterbitkan</h2>
            </div>
